...
class Pesan extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id, pesan, waktu, penerima, pengirim, id_user', 'required'),
			array('id, id_user', 'numerical', 'integerOnly'=>true),
			array('waktu', 'length', 'max'=>20),
			array('penerima, pengirim', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, pesan, waktu, penerima, pengirim, id_user', 'safe', 'on'=>'search'),
		);
	}
